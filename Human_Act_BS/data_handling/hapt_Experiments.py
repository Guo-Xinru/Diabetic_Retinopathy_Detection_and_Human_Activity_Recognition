...
class Experiment:
    ...
    def combine_sensor_values(self):
        ...
        # combine the two array acc and gyro with every corresponding column
        self._combined_sensor_values = np.concatenate((self._acceleration_values, self._gyroscope_values), axis=1)

    # the function is add label to the sensor data
    def add_labels_to_combined_data(self):
        # go through label-file-entries and adjust labels of combinedSensorData
        # create a list with the same length of combined sensor but all values are 0
        self._labels = [0] * self._combined_sensor_values.__len__()

        # go through the whole list of Experiment.label_frames
        # we can see in label.txt
        # first col : experiment number
        # third : label value
        # fourth: temp start stamp
        # fifth: temp end stamp
        # then set the  self._labels to the label value in corresponding stamp
    ...
    # the window size and shift_ratio are the hyper parameters
    # can be adjusted
    def slide_window(self, window_size, shift_ratio=0.5, labeling_threshold=0.5):
        ...
        # calculate how much windows can be devided
        for i in range(int(len(self._combined_sensor_values) / (window_size * shift_ratio))):
            # select the windows contains datas and labels
            ...
            # if the window's size not correct then dont append it into list
...
# the normal way X-norm = (X-mean)/std
